...
print (f"La funcion de trasnferencia del controlador PID es {PID}")


# Sistema de control en lazo cerrado
X = ctrl.series(PID,sys)
sysPID = ctrl.feedback(X,1,sign = -1)
print(f"La funcion de transferencia del sistema central en lazo cerrado {sysPID}") 

# Respuesta del sistema en lazo abierto y en lazo cerrado
clr1 = np.array([0.902,0.224,0.274])
clr2 = np.array([0.659,0.855,0.863])
clr6 = np.array([0.569,0.392,0.235])
...
plt.legend(bbox_to_anchor = (0.5,-0.2),loc = 'center', ncol = 3,
           fontsize = 9, frameon = True)
plt.show()
fgl.savefig('Impulse_python.pdf',bbox_inches = 'tight')
...
plt.legend(bbox_to_anchor = (0.5,-0.2),loc = 'center', ncol = 3,
           fontsize = 9, frameon = True)
plt.show()
fgl.savefig('ramp_python.pdf',bbox_inches = 'tight')
